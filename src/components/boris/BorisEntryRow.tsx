...
import BorisBookingInfoCard from './BorisBookingInfoCard';
import BorisCleaningCard from './BorisCleaningCard';
import { getBookingColorByIndex, STANDALONE_COLOR } from '@/lib/bookingColors';

interface BorisEntryRowProps {
  entry: any;
  staff: any[];
...
const BorisEntryRow: React.FC<BorisEntryRowProps> = ({
  entry,
  staff,
  onStatusUpdate,
  onStaffUpdate,
  ...
  if (tasks.length === 0) {
    return <BorisBookingInfoCard booking={booking} accentColor={accentColor} />;
  }
  ...
  return (
    <div className="space-y-2.5">
      <BorisBookingInfoCard booking={booking} accentColor={accentColor} />
      {isMulti && (
